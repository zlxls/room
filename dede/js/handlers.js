...
function uploadSuccess2(file, serverData) {
	try {
		...
		if (serverData.substring(0, 7) === "FILEID:") {
			addImage2("swfupload2.php?dopost=thumbnail&id=" + serverData.substring(7), serverData.substring(7));

			progress.setStatus("获取缩略图...");
			progress.toggleCancel(false);
		} else {
			addImage2("images/error.gif", 0);
			progress.setStatus("有错误！");
			progress.toggleCancel(false);
			alert(serverData);
...
function addImage2(src, pid){
	...
	newImgDiv.className = 'albCt';
	newImgDiv.id = 'albCt'+albImg;
	document.getElementById("thumbnails2").appendChild(newImgDiv);
	newImgDiv.innerHTML = '<img src="'+src+'" width="120" />'+delstr;
	if(typeof arctype != 'undefined' && arctype ==  'article' )
	{ 
		iptwidth = 100;
